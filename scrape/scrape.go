...
// After sl.append...
if scrapeErr == nil && buf != nil && buf.Len() > 0 {
		parser := textparse.NewPromParser(buf.Bytes(), nil, false)
		...
		for {
			...
			lbls, _, val := parser.Series()

			if (string(lbls)=="probe_success"){
               if val == 1 {
				// probe passed
               ...
               // probe failed
				 ...
				 scrapeErr = fmt.Errorf("blackbox probe failed (probe_success == 0)")
			     sl.l.Warn("Blackbox exporter target marked DOWN", "url", resp.Request.URL.String())
